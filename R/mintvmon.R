...
f=double(n),derivsign=integer(n-1),secsign=integer(n-1),as.integer(method),as.integer(DYADIC),as.double(thresh),as.double(y),as.integer(MONCONST),as.integer(CONVCONST),jact=integer(n),kact=integer(n),signact=integer(n),nact=integer(1),piecesleft=integer(n),piecesright=integer(n))
