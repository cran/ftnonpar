...
C
	ADJUST=.FALSE.
	CALL GGMLTRSTN(SY,RES,KNV,N,W,THRSH,KK,MULT,ADJUST,IJ)
	IC=IC+1
C     
	IF (ADJUST) THEN
...
C
C
      SUBROUTINE GGMLTRSTN(SY,RES,KNV,N,W,THRSH,KK,MULT,ADJUST,KIJ)
      DOUBLE PRECISION SY(N+1),RES(N),W(N+1),THRSH(2*KK+2)
      INTEGER KNV(N)
      INTEGER MULT
      LOGICAL ADJUST
C
